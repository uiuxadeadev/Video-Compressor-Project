...
# protocol_header()という関数は、
# サーバに送信されるファイルのヘッダ情報をフォーマットするために使用されます。
# このヘッダは、ファイル名の長さ（バイト）、
# JSONデータの長さ（バイト）、データの長さ（バイト）の3つの値で構成されます。
...
try:
    ...
    with open(filepath, 'rb') as f:
        # ファイルの末尾に移動し、tellは開いているファイルの現在位置を返します。
        # ファイルのサイズを取得するために使用します
        ...
        # 一度に4096バイト（4096ビット）ずつ読み出し、送信することにより、ファイルを送信します。
        # Readは読み込んだビットを返します
        data = f.read(4096)
        while data:
            print("Sending...")
